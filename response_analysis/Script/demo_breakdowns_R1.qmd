...
#### Overview & Goals 
**Create a function to save these histograms for each category as png in a folder to look at later**
...
generate_plot <- function(df, country, survey_question, round_of_surveying) {
  ...
  folder_path <- paste0("..","response_visuals",country, "_response_visuals")
  ...
  if (survey_question %in% names(df)) {
    ...
    if ("Yes" %in% df[[survey_question]]) {
      ...
      # Save plot as a png
      plot_filename <- paste0(folder_path, "/", country, "_", survey_question, "_response_analysis_", round_of_surveying, ".png")
      ggsave(filename = plot_filename, plot = plot_data, width = 10, height = 6, dpi = 300)
      
      print("Plot saved successfully for 'Yes' responses.")
    } else {
      # Subset data for plotting 
      plot_data_bgd <- df %>%
        # label groups by age demo
      ...
      # Save plot as a png
      plot_filename <- paste0(folder_path, "/", country, "_", survey_question, "_response_analysis_", round_of_surveying, ".png")
      ggsave(filename = plot_filename, plot = plot_data_bgd, width = 10, height = 6, dpi = 300)
      
      print("Plot saved successfully for 'Strongly Agree' responses.")
    }
  } else {
...
generate_plot <- function(df, country, survey_question, round_of_surveying) {
  # Create folder if it doesn't exist
  folder_path <- paste0(here("response_analysis","response_visuals",paste0(country, "_response_visuals")))
  if (!file.exists(folder_path)) {
    dir.create(folder_path)
    print(paste("Folder", folder_path, "created."))
  }
  ...
  if (survey_question %in% names(df)) {
    ...
    if ("Yes" %in% df[[survey_question]] | survey_question == "income_brackets") {
      # Subset data for plotting 
      plot_data <- df %>%
        # label groups by age demo
        mutate(age_group = fct_reorder(age_group, .data[[survey_question]])) %>%
...
generate_plot(plot_data_bgd, "BGD", "enough_eat", "R1")
```
#
